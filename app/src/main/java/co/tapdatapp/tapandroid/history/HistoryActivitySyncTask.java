...
public class HistoryActivitySyncTask
extends AsyncTask<HistoryActivity, Void, Void> {
    ...
    @Override
    protected Void doInBackground(HistoryActivity... historyActivities) {
        if (historyActivities.length != 1) {
            throw new AssertionError(
                "Must be called with single HistoryFragment"
            );
        }
        historyActivity = historyActivities[0];
        /*
    ...
    protected void onPostExecute(Void x) {
        if (success) {
            historyActivity.postSyncDisplay();
        }
        else {
            historyActivity.syncFailure(exception);
        }
    }
